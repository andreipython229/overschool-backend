...
def meeting_reminders_tg():
    ...
    try:
        ...
        for reminder in reminders:
            ...
            students = (
                session.execute(
                    select(students_table.c.user_id).filter(
            ...
            for student in students:
                ...
                tg_user_chat_id = (
                    session.execute(
                        select(tgusers_table.c.tg_user_id).where(
                ...
                start_date_query = (
                    session.execute(
                        select(school_meetings_table.c.start_date).where(
                ...
                if rem_daily is True and datetime.now(
                    timezone.utc
                ...
                if rem_in_three_hours is True and datetime.now(
                    timezone.utc
                ...
                if rem_in_ten_minute is True and datetime.now(
                    timezone.utc
