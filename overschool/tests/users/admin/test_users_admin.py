...
class TestUsersAdmin(AuthAdminTestCase):
    def test_users_changelist(self):
        ...
        response = self.client.get(
            reverse(admin_urlname(User._meta, "changelist")), format="json"
        )
    ...
    def test_users_change(self):
        ...
        response = self.client.get(
            reverse(admin_urlname(User._meta, "change"), args=[1]), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.content)
    ...
    def test_users_delete(self):
        ...
        response = self.client.get(
            reverse(admin_urlname(User._meta, "delete"), args=[1]), format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK, response.content)
