...
class CoursesTestCase(APITestCase):
    """Тест-кейс для эндпоинта /courses/"""
    ...
    def test_courses_get(self):
        url = reverse("courses-list", args=["School_1"])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    def test_courses_id_get(self):
        url = reverse("courses-detail", args=["School_1", self.course.pk])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_courses_id_sections_get(self):
        url = reverse("courses-sections", args=["School_1", self.course.pk])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
