...
class StudentsGroupViewSetAPITestCase(APITestCase):
    ...
    def test_create_students_group(self):
        url = reverse("students_group-list", args=["School_1"])
        post_data = {"name": "string", "course_id": 3, "teacher_id": 5, "students": [3]}
        response = self.client.post(url, post_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_retrieve_students_group(self):
        url = reverse(
            "students_group-detail", args=["School_1", self.students_group.pk]
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_students_group(self):
        ...
        put_data = {
            "group_settings": {"strict_task_order": True, "task_submission_lock": True},
            "name": "string",
            "course_id": 1,
            "teacher_id": 5,
            "students": [2],
    ...
    def test_delete_students_group(self):
        ...
        self.assertFalse(
            StudentsGroup.objects.filter(pk=self.students_group.pk).exists()
        )
