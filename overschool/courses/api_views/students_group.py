...
class UsersGroup(LoggingMixin, WithHeadersViewSet, generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        queryset = StudentsGroup.objects.filter(group_id=1)
...
class GroupUsersByMonthView(LoggingMixin, WithHeadersViewSet, generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        queryset = StudentsGroup.objects.filter(group_id=1, students__created_at__month=kwargs["month_number"])
        datas = queryset.values(group=F("group_id")).annotate(students_sum=Count("students__user_id"))
        for data in datas:
