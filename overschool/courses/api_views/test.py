...
class TestViewSet(LoggingMixin, WithHeadersViewSet, viewsets.ModelViewSet):
    ...
    def questions(self, request, pk):
        ...
        test = {"test": test_obj["test_id"], "name": test_obj["test_id"], "show_right_answers": test_obj["test_id"]}
        if test_obj["random_questions"]:
            questions = Question.objects.filter(test=pk).order_by("?").values("body", "question_id")
        else:
        ...
        for index, question in enumerate(questions):
            if test_obj["random_answers"]:
                answers = (
                    Answer.objects.filter(question=question["question_id"]).order_by("?").values("answer_id", "body")
                )
            else:
                answers = Answer.objects.filter(question=question["question_id"]).values("answer_id", "body")
