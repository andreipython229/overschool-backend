...
from courses.serializers import (TeacherHomeworkSerializer,
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        try:
            queryset = queryset.filter(
                Q(mark__gte=kwargs["start_mark"]) & Q(mark__lte=kwargs["end_mark"])
            )
        except KeyError:
            pass
        if kwargs["status"]:
        ...
        if kwargs["course_id"]:
            queryset = queryset.filter(
                homework__lesson__section__course__id=kwargs["course_id"]
            )
        if kwargs["group_id"]:
