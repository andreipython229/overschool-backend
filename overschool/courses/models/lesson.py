...
class Lesson(TimeStampedModel):
    ...
    name = models.CharField(
        max_length=256, verbose_name="Название урока", help_text="Название урока"
    )
    description = models.TextField(
        verbose_name="Описание", help_text="Описание к уроку"
    )
    video = EmbedVideoField(
        verbose_name="Видео",
        help_text="Сюда всталвяем ссылку на видос с ютуба, поэтому сначала его надо загрузить туда",
    )
    previous_lesson_id = models.ForeignKey(
        "self",
    ...
    def order(self):
        if self.previous_lesson_id:
            previous_lesson: Lesson = Lesson.objects.get(
                lesson_id=self.previous_lesson_id
            )
